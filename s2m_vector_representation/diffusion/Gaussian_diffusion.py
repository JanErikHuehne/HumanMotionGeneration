...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        x1 = x1.permute((1, 2, 0))
        model_output, _ = model(x1, sketch, keyframe, self._scale_timesteps(t), **model_kwargs)
    ...
    def training_losses(self, model, x_start, t, sketch=None, keyframe= None, model_kwargs=None, noise=None, dataset=None):
        ...
        model_output, predict_vectors = model(x_t, sketch, keyframe, self._scale_timesteps(t), **model_kwargs)
        target = x_start
        ...
        ###additional loss
        # loss1 = self.l2_loss(sketch.reshape(sketch.shape[0], sketch.shape[1], -1), predict_vectors)
        # loss1 = loss1.sum(dim=list(range(1, len(loss.shape))))
        # loss1 = loss1 / (predict_vectors.shape[1] * predict_vectors.shape[2])
        # terms["loss"] += 10 * loss1
        return terms

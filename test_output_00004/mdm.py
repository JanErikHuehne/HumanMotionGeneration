...
class MDM(nn.Module):
    ...
    def forward(self, x, y, key_frames,timesteps):
        ...
        #######
        time_emb = time_emb.repeat(3, 1, 1) # yields [3, bs, d]
        #######
        #print("TIMESTEP EMBEDDING", emb.shape)
        ######
        y['pixel_values'] = y['pixel_values'].reshape(-1, 3, 224, 224)
        sketches_emb = self.sketchEncoder.get_image_features(**y)
        #######
        sketches_emb = self.linear_layer(sketches_emb)
        ########
        sketches_emb = sketches_emb.reshape(-1, 3, 512) # [bs, 3(sketches), d]
        sketches_emb = sketches_emb.permute(1, 0, 2) #[3(sketches), bs, d]
        ######
        ...
        xseq = torch.cat((emb, x), axis=0)  # [seqlen+3, bs, d]
        xseq = self.sequence_pos_encoder(xseq) # [seqlen+3, bs, d]
        output = self.seqTransEncoder(xseq)[3:] # [seqlen, bs, d]

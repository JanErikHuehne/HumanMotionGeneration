...
def main():
    loader = get_dataset_loader(datapath='test_data/humanml_opt.txt', batch_size=3, split="train")
    model = CLIPModel.from_pretrained("openai/clip-vit-base-patch16")
    device = th.device("cuda" if th.cuda.is_available() else "cpu")
    for param in model.parameters():
        param.requires_grad = False
    model = model.to(device)
    for i, (motion, sketches, key_frames) in enumerate(loader):
        # sketch.to(dev())
        ...
        print(outputs.shape)
        print(outputs)
        outputs = outputs.reshape(-1, 3, 512)
        zero_indices = (sketches != 1).nonzero(as_tuple=True)
        print(zero_indices)

...
class Sketch2MotionDataset(data.Dataset):
    ...
    def __init__(self, mean, std, opt, data_file, transform=True, top_10 = True):
        ...
        for name in tqdm(id_list):
            ...
            for i in range(motion_length // 10 - 4):
                sketch_crop = [sketches[i], sketches[i+2], sketches[i+4]]
    ...
    def __getitem__(self, index):
        ...
        sketches = []
        key_frames = []
        for sketch_name, key_frame in data["sketches"]:
            sketch = Image.open(sketch_name)
            sketches.append(sketch)
            key_frames.append(key_frame)
        key_frames = torch.tensor(key_frames)
        sketches = self.transform_img(sketches)
        motion = motion[self.data_dict3[index]['motion_crop'][0]:self.data_dict3[index]['motion_crop'][1]]

...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        timestep = t[0].item()
        total_length = out["mean"].shape[-1]
        if timestep > 500 and cond_frames is not None:
         
            rel_frames = cond_frames["frames"]
            for index, id in enumerate(rel_frames):
                cond = cond_frames["cond"][..., index]    
                id = int(total_length * id)
                if id == total_length:
                    id -= 1
                if id < 5:
                    out["mean"][:, :, :, :id+10] = cond[..., np.newaxis]
                elif id > total_length - 5:
                    out["mean"][:, :, :, id-10:] = cond[..., np.newaxis]
                else:
                    out["mean"][:, :, :, id-5:id+5] = cond[..., np.newaxis]
        """        
        elif cond_frames and timestep > 0:
            rel_frames = cond_frames["frames"]
            for index, id in enumerate(rel_frames):
                cond = cond_frames["cond"][..., index] 
                id = int(total_length * id)
                if id == total_length:
                    id -= 1
        """  
                #out["mean"][:, 24, [0,2], id] = cond[np.newaxis, 24, [0,2]]
                #out["mean"][:, 24, [3,5], id] = cond[np.newaxis, 24, [0,2]]

...
def get_collate_fn(name, hml_mode='train'):
    print("COLLATE FN", name, hml_mode)
    if hml_mode == 'gt':
...
def get_dataset_loader(name, batch_size, num_frames, split='train', hml_mode='train'):
    ...
    for x, y in loader:
        t = x.detach().numpy()
        import numpy as np
        name = y['y']["action_text"][0]
        np.save(f"/media/jan/SSD Spiele/ADLCV/HumanMotionGeneration/ucs2m/dataset/conditioning_test/{name}.npy", t)
        print("SAVED")
    """
    return loader
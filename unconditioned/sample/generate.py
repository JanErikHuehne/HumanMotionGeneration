...
from sample.generate_utils import preprocess_inputs
from sample.translation_selection import ClickCounterApp
...
def main():
    ...
    conditions = args.frames_condition
    if conditions is not None: 
        
        conditions = preprocess_inputs(conditions)
        sketch_number = len(conditions["frames"])
        #app = ClickCounterApp(mc=sketch_number)
        #app.run()
        
        #translation = torch.tensor(app.run())
        #print(conditions["cond"][0, 4:, :].shape)
        #print(translation.view(2,sketch_number).shape)
        #conditions["cond"][24, [0,2], :] =  translation.view(2,sketch_number).float()
        #conditions["cond"] = conditions["cond"].to(dist_util.dev())
        #print(conditions["cond"].get_device())
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = sample_fn(
            ...
            dump_steps=None,
            noise=None,
            cond_frames = conditions,
            const_noise=False,
        )
        print("SAMPLE OUTPUT ",  sample.shape)
        print("Model data rep ", model.data_rep)
        ...
        print("Sa mple shape ", sample.shape)
...
def save_multiple_samples(args, out_path, row_print_template, all_print_template, row_file_template, all_file_template,
    ...
    if (sample_i + 1) % num_samples_in_out_file == 0 or sample_i + 1 == args.num_samples:
        ...
        print("SAVE PATH ", all_sample_save_path)
